...
def upload():
    if request.method == 'POST':
        ...
        df['Last Name'] = df['Last Name'].fillna('-9999')
        df['Employees'] = df['Employees'].fillna('-9999')
        df['Annual Revenue'] = df['Annual Revenue'].fillna('-9999')
        df['Industry'] = df['Industry'].fillna('-9999')
        df['Company Address'] = df['Company Address'].fillna('-9999')
        df['Company City'] = df['Company City'].fillna('-9999')
        df['Company State'] = df['Company State'].fillna('-9999')
        df['Company Phone'] = df['Company Phone'].fillna('-9999')
        df['Company Country'] = df['Company Country'].fillna('-9999')
        df['Person Linkedin Url'] = df['Person Linkedin Url'].fillna('-9999')
        df['Company Linkedin Url'] = df['Company Linkedin Url'].fillna('-9999')
        df['Facebook Url'] = df['Facebook Url'].fillna('-9999')
        df['Twitter Url'] = df['Twitter Url'].fillna('-9999')
        df['Keywords'] = df['Keywords'].fillna('-9999')
        df['SEO Description'] = df['SEO Description'].fillna('-9999')
        df['Technologies'] = df['Technologies'].fillna('-9999')
        df['Company Linkedin Url'] = df['Company Linkedin Url'].fillna('-9999')
        df['Facebook Url'] = df['Facebook Url'].fillna('-9999')
        ...
        df=df.drop(index=Last_toosingle_index,axis='rows')
        ...
        title_num2=df.loc[(df['Title'].str.contains(pat='[0-9][0-9][0-9]',regex=True))]
        title_num2_index=df.loc[df['Title'].str.isnumeric()].index
        ...
        Last_toosingle=Last_toosingle.append(title_num2)
        numeric_title=df.loc[df['Title'].str.contains(pat = '[0-9]\s', regex = True)]
        numeric_title_index=df.loc[df['Title'].str.contains(pat = '[0-9]\s', regex = True)].index
        numeric_title2=df.loc[df['Title'].str.contains(pat = '[0-9][0-9]', regex = True)]
        numeric_title2_index=df.loc[df['Title'].str.contains(pat = '[0-9][0-9]', regex = True)].index
        numeric_title['remarks']='Numeric data in Titles'
        numeric_title2['remarks']='Numeric data in Titles'
        Error_list=Error_list.append(numeric_title)
        Error_list=Error_list.append(numeric_title2)
        try:
            df=df.drop(index=title_num_index,axis='rows')
            df=df.drop(index=title_single_index,axis='rows')
            df=df.drop(index=title_num2_index,axis='rows')
            df=df.drop(index=numeric_title_index, axis=0)
            df=df.drop(index=numeric_title2_index, axis=0)
   ...
   #         df['Title'] = df['Title'].str.replace(char, ' ')
        ...
        df.loc[df['Title'].str.contains(pat = 'distributor', regex = True),'Seniority Level']= 'Entry'
        ...
        df.loc[df['Title'].str.contains(pat = 'engineer\s', regex = True),'Seniority Level']= 'Senior'
        ...
        df.loc[df['Title'].str.contains(pat = 'principal', regex = True),'Seniority Level']= 'Senior'
        df.loc[df['Title'].str.contains(pat = 'principle', regex = True),'Seniority Level']= 'Senior'
        ...
        df['Title']=df['Title'].replace([0-9],'',regex=True)
        # CEO
        df['Title']=df['Title'].replace('^ceo\s','Chief executive officer ',regex=True)
        df['Title']=df['Title'].replace('\sceo\s','Chief executive officer ',regex=True)
        df['Title']=df['Title'].replace('\sceo$','Chief executive officer ',regex=True)
        df['Title']=df['Title'].replace('^ceo$','Chief executive officer ',regex=True)
        df['Title']=df['Title'].replace('ceo,','Chief executive officer ',regex=True)
        df['Title']=df['Title'].replace('^ceo$','Chief executive officer ',regex=True)

        df['Title'] = df['Title'].str.split('\sat\s').str[0]# split on 'at' and delete later
        # Senior
        df['Title']=df['Title'].replace('^sr,','Senior ',regex=True)
        df['Title']=df['Title'].replace('^sr\s,','Senior ',regex=True)
        df['Title']=df['Title'].replace('\ssr\s,','Senior ',regex=True)
        df['Title']=df['Title'].replace('^sr\s','Senior ',regex=True)
        df['Title']=df['Title'].replace('\ssr,','Senior ',regex=True)
        df['Title']=df['Title'].replace('^sr$,','Senior ',regex=True)

        # VP
        df['Title']=df['Title'].replace('^svp\s','Senior vice president ',regex=True)
        df['Title']=df['Title'].replace('^svp,','Senior vice president ',regex=True)
        df['Title']=df['Title'].replace('\ssvp$','Senior vice president ',regex=True)
        df['Title']=df['Title'].replace('\ssvp\s','Senior vice president ',regex=True)
        df['Title']=df['Title'].replace('^svp$','Senior vice president ',regex=True)

        df['Title']=df['Title'].replace('^avp\s','Assistant vice president ',regex=True)
        df['Title']=df['Title'].replace('^avp,','Assistant vice president ',regex=True)
        df['Title']=df['Title'].replace('\savp$','Assistant vice president ',regex=True)
        df['Title']=df['Title'].replace('\savp\s','Assistant vice president ',regex=True)
        df['Title']=df['Title'].replace('^avp$','Assistant vice president ',regex=True)
        df['Title']=df['Title'].replace('\savp,','Assistant vice president ',regex=True)

        df['Title']=df['Title'].replace('^vp\s','Vice president ',regex=True)
        df['Title']=df['Title'].replace('^vp,','Vice president ',regex=True)
        df['Title']=df['Title'].replace('\svp,','Vice president ',regex=True)
        df['Title']=df['Title'].replace('\svp$','Vice president ',regex=True)
        df['Title']=df['Title'].replace('\svp\s','Vice president ',regex=True)
        df['Title']=df['Title'].replace('^vp$','Vice president ',regex=True)
        df['Title']=df['Title'].replace('Seniorvp','Senior vice president ',regex=True)

        df['Title']=df['Title'].replace('^evp\s','Executive vice president ',regex=True)
        df['Title']=df['Title'].replace('^evp,','Executive vice president ',regex=True)
        df['Title']=df['Title'].replace('\sevp$','Executive vice president ',regex=True)
        df['Title']=df['Title'].replace('\sevp\s','Executive vice president ',regex=True)
        df['Title']=df['Title'].replace('^evp$','Executive vice president ',regex=True)

        # CTO
        df['Title']=df['Title'].replace('^cto\s','Cheif technical officer ',regex=True)
        df['Title']=df['Title'].replace('^vp,','Cheif technical officer ',regex=True)
        df['Title']=df['Title'].replace('\scto$','Cheif technical officer ',regex=True)
        df['Title']=df['Title'].replace('\scto\s','Cheif technical officer ',regex=True)
        df['Title']=df['Title'].replace('^cto$','Cheif technical officer ',regex=True)

        # Manager
        df['Title']=df['Title'].replace('^mgr\s','Manager ',regex=True)
        df['Title']=df['Title'].replace('^mgr,','Manager ',regex=True)
        df['Title']=df['Title'].replace('\smgr$','Manager ',regex=True)
        df['Title']=df['Title'].replace('\smgr\s','Manager ',regex=True)
        df['Title']=df['Title'].replace('^mgr$','Manager ',regex=True)

        # Hr
        df['Title']=df['Title'].replace('^hr\s','Human resource ',regex=True)
        df['Title']=df['Title'].replace('^hr,','Human resource ',regex=True)
        df['Title']=df['Title'].replace('\shr$','Human resource ',regex=True)
        df['Title']=df['Title'].replace('\shr\s','Human resource ',regex=True)
        df['Title']=df['Title'].replace('^hr$','Human resource ',regex=True)

        # gm
        df['Title']=df['Title'].replace('^gm\s','General Manager ',regex=True)
        df['Title']=df['Title'].replace('^gm,','General Manager ',regex=True)
        df['Title']=df['Title'].replace('\sgm$','General Manager ',regex=True)
        df['Title']=df['Title'].replace('\sgm\s','General Manager ',regex=True)
        df['Title']=df['Title'].replace('^gm$','General Manager ',regex=True)

        # COO
        df['Title']=df['Title'].replace('^coo\s','Chief operating officer ',regex=True)
        df['Title']=df['Title'].replace('\scoo\s','Chief operating officer ',regex=True)
        df['Title']=df['Title'].replace('\scoo$','Chief operating officer ',regex=True)
        df['Title']=df['Title'].replace('^coo$','Chief operating officer ',regex=True)
        df['Title']=df['Title'].replace('coo,','Chief operating officer ',regex=True)
        df['Title']=df['Title'].replace('^coo$','Chief operating officer ',regex=True)

        # COO
        df['Title']=df['Title'].replace('^cfo\s','Chief finance officer ',regex=True)
        df['Title']=df['Title'].replace('\scfo\s','Chief finance officer ',regex=True)
        df['Title']=df['Title'].replace('\scfo$','Chief finance officer ',regex=True)
        df['Title']=df['Title'].replace('^cfo$','Chief finance officer ',regex=True)
        df['Title']=df['Title'].replace('cfo,','Chief finance officer ',regex=True)
        df['Title']=df['Title'].replace('^cfo$','Chief finance officer ',regex=True)



        department_others_list=['owner','president','manager','partner','head','Chief executive officer','Senior vice president','Executive vice president','director','founder','office manager',
                        'president & ceo','plant manager','vice president','agent','co-founder','Cheif technical officer','General Manager','chief executive officer','Chief Executive Officer',
                        'Chief Finance Officer','associate','supervisor']
        ...
        df['Company City']= df['Company City'].str.replace('9999','')
        df['Company State']= df['Company State'].str.replace('9999','')
        df['Company Country']= df['Company Country'].str.replace('9999','')

        df = df[['First Name', 'Last Name', 'Title','Email','Seniority Level','Department',  'Company','Website', 'Employees Size',
            ...
            'Company Linkedin Url','Facebook Url', 'Twitter Url', 'Keywords','SEO Description', 
            'Technologies' ]]
